...
func getPosts(w http.ResponseWriter, r *http.Request) {
	...
	// err = db.Select(&results, "SELECT `id`, `user_id`, `body`, `mime`, `created_at` FROM `posts` WHERE `id` <= (SELECT `id` FROM `posts` WHERE `created_at` >= ?  LIMIT 1) ORDER BY `created_at` DESC", t.Format(ISO8601Format))
	if err != nil {
		log.Print(err)
		return
	}
